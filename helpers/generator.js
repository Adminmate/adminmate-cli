...
const createAmConfigFile = (projectPath, models) => {
  const tplContent = fs.readFileSync(appRoot('/app-template/config/adminmate.hbs'), 'utf8');
...
const createModelFile = (projectPath, database, model) => {
  if (['mysql', 'sqlite', 'postgresql'].includes(database)) {
    ...
    const tplContent = fs.readFileSync(appRoot('/app-template/models/schema-mongoose.hbs'), 'utf8');
    const compiledTplContent = handlebars.compile(tplContent);
...
const createDatabaseFile = (projectPath, database) => {
  if (['mysql', 'postgresql', 'sqlite'].includes(database)) {
    let tplContent = fs.readFileSync(appRoot('/models-tmp/init-models.js'), 'utf8');
    ...
    fs.rmdirSync(appRoot('/models-tmp`, { recursive: true });
  }
  fs.copyFileSync(appRoot(`/app-template/database-${databaseTemplates[database]}.js`), `${projectPath}/database.js`);
};

const createServerJsFile = (projectPath, databaseType) => {
  const serverJsContent = fs.readFileSync(appRoot('/app-template/server.hbs'), 'utf8');
