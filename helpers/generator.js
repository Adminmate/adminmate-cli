...
const createAdminTemplate = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const targetDir = global.use_local_cli
      ? cwd.replace("/adminmate-cli", "")
      : cwd;
    const projectSlug = `${slugify(
      projectName
    ).toLocaleLowerCase()}-adminmate-api`;
...
const createAmConfigFile = (projectPath, models) => {
  const tplContent = fs.readFileSync(
    reqlib("/app-template/config/adminmate.hbs"),
    "utf8"
  );
...
const createModelFile = (projectPath, database, model) => {
  if (["mysql", "sqlite", "postgresql"].includes(database)) {
    ...
    const tplContent = fs.readFileSync(
      reqlib("/app-template/models/schema-mongoose.hbs"),
      "utf8"
    );
    const compiledTplContent = handlebars.compile(tplContent);
...
const createDatabaseFile = (projectPath, database) => {
  if (["mysql", "postgresql", "sqlite"].includes(database)) {
    let tplContent = fs.readFileSync(
      reqlib("/models-tmp/init-models.js"),
      "utf8"
    );
    ...
    fs.rmdirSync(reqlib("/models-tmp"), { recursive: true });
  }
  fs.copyFileSync(
    reqlib(`/app-template/database-${databaseTemplates[database]}.js`),
    `${projectPath}/database.js`
  );
};

const createServerJsFile = (projectPath, databaseType) => {
  const serverJsContent = fs.readFileSync(
    reqlib("/app-template/server.hbs"),
    "utf8"
  );
